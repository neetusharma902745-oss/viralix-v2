...
document.querySelectorAll('.like-btn').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    if (btn.classList.contains('liked')) {
      ...
      span.textContent='Like'; span.style.color='';
    } else {
      btn.classList.add('liked');
      svg.setAttribute('fill','var(--rd)'); svg.setAttribute('stroke','var(--rd)');
      span.textContent='Liked'; span.style.color='var(--rd)';
...
if (submitBtn) {
  submitBtn.addEventListener('click', async () => {
    ...
    if (!content) { alert('Kuch likhein pehle!'); return; }
    try {
      ...
      if (data.ok) {
        ...
        div.className='post'; div.dataset.id=data.post.id;
        div.innerHTML=`
          <div class="post-head">
            <div class="post-av">😊</div>
            <div style="flex:1"><div class="post-name">Aap (You)</div><div class="post-time">Abhi · Meerut, UP</div></div>
          </div>
          <div class="post-body">${content.replace(/</g,'&lt;').replace(/\n/g,'<br>').replace(/#(\w+)/g,'<span style="color:var(--ac)">#$1</span>')}</div>
          <div class="post-stats">0 likes · 0 comments · 0 shares</div>
          <div class="post-actions">
            <div class="pact like-btn" data-id="${data.post.id}">
        ...
        div.querySelector('.like-btn').addEventListener('click', function(){
          ...
          if(this.classList.contains('liked')){this.classList.remove('liked');s.setAttribute('fill','none');s.setAttribute('stroke','currentColor');sp.textContent='Like';sp.style.color='';}
          else{this.classList.add('liked');s.setAttribute('fill','var(--rd)');s.setAttribute('stroke','var(--rd)');sp.textContent='Liked';sp.style.color='var(--rd)';}
        });
